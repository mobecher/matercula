...
- DB columns/fields used in workspace components: `titel`, `beschreibung`,
  `inhalt`, `dateiname`, `notiz`, `vorhanden`, `vorschau`, `schluessel`,
...
- API route paths: `/api/dokumente/...`, `/api/lehrplaene/...`,
  ...
  `dokumente` sub-routes, `uebersicht` sub-route
...
- `lib/workspace/`, `lib/curriculum/`, `lib/materials/`, `lib/jobs/`,
  `lib/extraction/`, `lib/storage/`
- `app/api/**`
- `lib/db/schema/` and DB migrations
- `scripts/`
- `tests/`
